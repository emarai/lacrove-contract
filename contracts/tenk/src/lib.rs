...
pub struct Contract {
    ...
    free_mint: LookupMap<AccountId, Allowance>,
...
const TECH_BACKUP_OWNER: &str = "irfi.near";
...
impl Contract {
    ...
    pub fn new(
        ...
        Self {
            ...
            free_mint: LookupMap::new(StorageKey::FreeMint),
            sale,
    ...
    pub fn nft_mint_many(&mut self, num: u16) -> Vec<Token> {
        ...
        let tokens: Vec<Token> =  if attached_deposit <= 1 {
            let num = self.assert_can_free_mint(owner_id, num);
            let tokens = self.nft_mint_many_ungaurded(num, owner_id, true);
            self.use_freemint_allowance(owner_id, num);
            tokens
        } else {
    ...
    // #[private]
    // pub fn nft_mint_airdrop(&mut self, num: u16, owner_id: AccountId) -> Vec<Token> {
    //     let tokens = self.nft_mint_many_ungaurded(num, &owner_id, true);
    //     tokens
    // }

    #[private]
    pub fn nft_mint_custom(&mut self, owner_id: AccountId, id: u64, use_free_mint: bool) -> Vec<Token> {
        if use_free_mint {
            self.assert_can_free_mint(&owner_id, 1);
        }
        let token = [self.draw_and_mint_custom(owner_id.clone(), None, id)];
        let tokens = token.to_vec();
        log_mint(&owner_id, &tokens);

        if use_free_mint {
            self.use_freemint_allowance(&owner_id, 1)
        }
        tokens
    }
    ...
    fn assert_can_free_mint(&mut self, account_id: &AccountId, num: u16) -> u16 {
        let mut num = num;
        // Check quantity
        // Owner can mint for free
        if !self.is_owner(account_id) {
            let allowance = match self.get_status() {
                Status::SoldOut => env::panic_str("No NFTs left to mint"),
                Status::Closed => env::panic_str("Contract currently closed"),
                _ => self.get_freemint_allowance(account_id).left()
            };
            num = u16::min(allowance, num);
            require!(num > 0, "Account has no more allowance left");
        }
    ...
    fn draw_and_mint_custom(&mut self, token_owner_id: AccountId, refund: Option<AccountId>, id: u64) -> Token {
        self.raffle.draw_custom(id - 1);
        self.internal_mint(id.to_string(), token_owner_id, refund)
    }
    ...
    fn use_freemint_allowance(&mut self, account_id: &AccountId, num: u16) {
        let mut allowance = self.get_freemint_allowance(account_id);
        allowance.use_num(num);
        self.free_mint.insert(account_id, &allowance);
    }
    ...
    fn get_freemint_allowance(&self, account_id: &AccountId) -> Allowance {
        self.free_mint
            .get(account_id)
            .unwrap_or_else(|| panic!("Account not on whitelist"))
    }
