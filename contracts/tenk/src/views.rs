...
impl Contract {
    ...
    pub fn freeminted(&self, account_id: &AccountId) -> bool {
        self.free_mint.contains_key(account_id)
    }

    pub fn get_freemint_info(&self, account_id: &AccountId) -> Option<Allowance> {
        self.free_mint.get(&account_id)
    }
    ...
    pub fn get_user_sale_info(&self, account_id: &AccountId) -> UserSaleInfo {
        ...
        let free_mint_allowance: u16  = self.free_mint.get(account_id).unwrap_or(Allowance {max: 0, used: 0}).left();
        UserSaleInfo {
            sale_info,
            remaining_allowance,
            is_vip: self.whitelisted(account_id),
            free_mints: Some(free_mint_allowance),
        }
    }
