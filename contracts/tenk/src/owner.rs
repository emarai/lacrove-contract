...
impl Contract {
    ...
    pub fn add_freemint_accounts(
        &mut self,
        accounts: HashMap<AccountId, u16>,
    ) -> bool {
        self.assert_owner_or_admin();
        accounts.iter().for_each(|(account_id, allowance_per_user)| {
            let allowance = self
                .free_mint
                .get(account_id)
                .unwrap_or_else(|| Allowance::new(*allowance_per_user))
                .raise_max(*allowance_per_user);
            self.free_mint.insert(account_id, &allowance);
            self.pending_tokens += *allowance_per_user as u32;
        });
        true
    }

    pub fn remove_freemint_account(&mut self, account_id: AccountId) -> Option<u16> {
        self.assert_owner_or_admin();
        self.free_mint.remove(&account_id).as_ref().map(Allowance::left)
    }
